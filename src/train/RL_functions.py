...
def padder_batch(batch):
  ...
  batch_tensors_padded = [torch.cat([t, t.new_zeros(max_len - len, t.size(-1))]) for (t, len) in zip(batch_tensors, len_episodes)]
...
def REINFORCE(train_dataset, policy_network, special_tokens, args, optimizer, device, logger, log_interval=10, store_episodes=True):
