...
def train(env, agent, log_interval=10, num_episodes=100, pretrain=False, reduced_vocab=False):
    running_reward = 0
    for i_episode in range(num_episodes):
        ...
        for t in range(0, env.max_len + 1):
            ...
            action, log_probs, value = agent.select_action(state, forced=forced, valid_actions=dict_tokens)
...
def test(env, agent, log_interval=1, num_episodes=10, reduced_vocab=False):
    running_reward = 0
    for i_episode in range(num_episodes):
        state, ep_reward = env.reset(), 0
        dict_tokens, reduced_vocab = env.get_reduced_action_space() if reduced_vocab else None, None
...
if __name__ == '__main__':
    ...
    train(env=env, agent=agent, log_interval=args.log_interval, num_episodes=args.num_episodes_train,
          ...
          reduced_vocab=args.reduced_vocab)
    logging.info("-" * 20)
    logging.info("TEST")
    logging.info("-" * 20)
    ...
    test(env=env, agent=agent, reduced_vocab=args.reduced_vocab, num_episodes=args.num_episodes_test)
