...
def mask_inf_truncature(valid_actions, logits, device, num_tokens=86):
    ...
    mask = mask.scatter_(-1, valid_actions, torch.gather(logits, -1, valid_actions))
...
class Truncation:
    ...
    def get_valid_actions(self, state, truncation, temperature):
        ...
        with torch.no_grad():
            log_probas_lm, logits_lm, origin_log_probs_lm = self.language_model.forward(state.to(self.device),
                                                                                        temperature)
...
class TopK(Truncation):
    ...
    def trim_zero_probabilities(self, top_k_weights, top_k_indices):
        ...
        assert trimmed_top_k_indices.shape[-1] == non_zero_probs.shape[
            -1], "error when trimming top_k_valid action space"
