    ...
    if args.env == "clevr":
        ...
        if device.type == "cpu":
            env = VQAEnv(args.data_path, features_h5path=os.path.join(args.data_path, "coco_trainval.lmdb"),
                         max_len=args.max_len, reward_type=args.reward, mode="mintrain", max_seq_length=23,
            ...
            test_envs = [VQAEnv(args.data_path, features_h5path=os.path.join(args.data_path, "coco_trainval.lmdb"),
                                max_len=args.max_len, reward_type=args.reward, mode=mode, max_seq_length=23,
                                debug=args.debug, diff_reward=args.diff_reward, reward_path=args.reward_path,
                                reward_vocab=args.reward_vocab, mask_answers=args.mask_answers)
                         for mode in test_modes]
